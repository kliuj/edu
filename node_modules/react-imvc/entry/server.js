...
function createExpressApp(config) {
  ...
  app.use('/mock', function (req, res, next) {
    var filePath = _path2.default.join(config.root, config.src, req.url + '.json');
  ...
  if (config.webpackDevMiddleware) {
    ...
    // publish 目录启动
    app.use(config.staticPath, _express2.default.static(_path2.default.join(config.root, config.static)));

    // 在根目录启动
    app.use(config.staticPath, _express2.default.static(_path2.default.join(config.root, config.publish, config.static)));
